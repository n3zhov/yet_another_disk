...
build-impl-%: build_%/Makefile
	@cmake --build build_$* -j $(NPROCS) --target yet_another_disk

# test
test-impl-%: build-impl-%
	@cmake --build build_$* -j $(NPROCS) --target yet_another_disk_unittest
	@cmake --build build_$* -j $(NPROCS) --target yet_another_disk_benchmark
	@cd build_$* && ((test -t 1 && GTEST_COLOR=1 PYTEST_ADDOPTS="--color=yes" ctest -V) || ctest -V)
	@pep8 tests

# testsuite service runner
service-impl-start-%: build-impl-%
	@cd ./build_$* && $(MAKE) start-yet_another_disk
...
install-debug: build-debug
	@cd build_debug && \
		cmake --install . -v --component yet_another_disk
...
--debug-start-in-docker: install
	@sed -i 's/config_vars.yaml/config_vars.docker.yaml/g' /home/user/.local/etc/yet_another_disk/static_config.yaml
	@psql 'postgresql://user:password@service-postgres:5432/yet_another_disk_db-1' -f ./postgresql/data/initial_data.sql
	@/home/user/.local/bin/yet_another_disk \
		--config /home/user/.local/etc/yet_another_disk/static_config.yaml
...
docker-impl-%:
	docker-compose run --rm yet_another_disk-service make $*

# Build and runs service in docker environment
docker-start-service-debug:
	@docker-compose run -p 8080:8080 --rm yet_another_disk-service make -- --debug-start-in-docker-debug

# Build and runs service in docker environment
docker-start-service:
	@docker-compose run -p 8080:8080 --rm yet_another_disk-service make -- --debug-start-in-docker
